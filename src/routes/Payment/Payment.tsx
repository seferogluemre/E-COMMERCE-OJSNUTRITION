...
import { BsTruck } from "react-icons/bs"
import "./Payment.scss"
import { handlePaymentSubmit, type PaymentData } from "../../services/api/collections/Payment"
import { useNavigate } from "react-router-dom"
import { PHOTO_URL } from "../../services/api/collections/Auth"
...
function Payment() {
  ...
  const totalAmount = cartItems.reduce((sum, item) => sum + item.price * (item.pieces || 1), 0)
        ...
        <>
          {!editingAddress && !showNewAddressForm ? (
            ...
            <Form onSubmit={handleNewAddressSubmit}>
              <div className="d-flex justify-content-between align-items-center mb-3">
                <h5 className="mb-0">Yeni Adres Ekle</h5>
                <Button variant="link" className="text-secondary p-0" onClick={() => setShowNewAddressForm(false)}>
                  Vazgeç
                </Button>
              </div>
            ...
            <Form onSubmit={handleFormSubmit}>
              <div className="d-flex justify-content-between align-items-center mb-3">
                <h5 className="mb-0">Adres Düzenle</h5>
                <Button variant="link" className="text-secondary p-0" onClick={() => setEditingAddress(null)}>
                  Vazgeç
                </Button>
              </div>
        ...
        <Form onSubmit={handleFormSubmit}>
          ...
          <Row className="mb-3">
            <Col md={6}>
              <Form.Group>
                <Form.Label>Şehir</Form.Label>
                <Form.Select name="city" value={formData.city} onChange={handleCityChange} required>
                  <option value="">Şehir Seçiniz</option>
                  {/* Add your cities here */}
                </Form.Select>
              </Form.Group>
          ...
          <Form.Group className="mb-3">
            <Form.Label>Tam Adres</Form.Label>
            <Form.Control as="textarea" rows={3} name="full_address" onChange={handleInputChange} required />
          </Form.Group>
            ...
            <div className="d-flex justify-content-between mt-3">
              ...
              {activeStep === 3 && (
                <Button
                  ...
                  className="ms-auto"
                  onClick={handleSubmitPayment}
                  disabled={isLoading || !cardInfo.cardNumber || !cardInfo.expirationDate || !cardInfo.cvv}
                >
              ...
              <Card.Body>
                ...
                {cartItems.map((item) => (
                  <div key={item.id} className="mb-3 pb-3 border-bottom">
                    <div className="d-flex gap-3">
                      <Image
                        src={PHOTO_URL + item.photo_src || "/placeholder.svg"}
                        alt={item.name}
                        style={{ width: "80px", height: "80px", objectFit: "cover" }}
                        rounded
                      />
